...
func RunUpdaterWithLog(oldDir, newExe, newDir, logFile string) error {
	...
	if updateErr != nil {
		...
		_ = zenity.Error(fmt.Sprintf("Update failed: %v", updateErr), zenity.Title("EDx52Display Updater"))
	...
	_ = zenity.Info("Update complete! Starting new version.", zenity.Title("EDx52Display Updater"))
	...
	if err := cmd.Start(); err != nil {
		_ = zenity.Error(fmt.Sprintf("Failed to start new exe: %v", err), zenity.Title("EDx52Display Updater"))
